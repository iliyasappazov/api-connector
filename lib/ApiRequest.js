...
export default class ApiRequest {
   ...
   * Axios response object will be passed to the function, and if `true` is returned the response is said to be
   * *successful*. If the function is `null` any response will be considered as *successful*.
   *
  ...
  startSingleQuietly(identifier = '') {
    this.startSingle(identifier)
      .then()
      .catch(() => {});
   ...
   * + `onFail` event will reject the Promise with an error with `isFail` property set `true` and `data` property with
   * a data returned by the last `onFail` handler.
   * + `onCancel` event will reject the Promise with an error with `isCancel` property set `true` and `data` property
   * with a data returned by the last `onCancel` handler.
   ...
   * @returns {Promise} request cancel function
   */
  startSingle(identifier = '') {
    ...
    if (pendingRequests) {
      const cancelFunctions = Object.values(pendingRequests);
      for (let i = 0, l = cancelFunctions.length; i < l; i += 1) {
        cancelFunctions[i](); // cancel request
      }
    }
   ...
   *
   * @param {string|number} identifier - some identifier
   * @returns {Promise} request cancel function, or Promise
   */
  start(identifier = '') {
    ...
    this.cancelFunction = source.cancel;

    return this.genPromise();
  }
  ...
  genPromise() {
    return new Promise((resolve, reject) => {
      this.onOk((data) => {
        ...
        const error = new Error('request response validation failed');
        error.isFail = true;
        error.data = data;
        reject(error);
        return data;
